...
import { PushPopType, } from '../../../../Runtime/PushPopType';
import { VariablePointerValue, } from '../../../../Runtime/Value/VariablePointerValue';
export class Divert extends Object {
    constructor(target, args = null) {
                ...
                if (requiresArgCodeGen) {
                    ...
                    for (let ii = 0; ii < this.args.length; ++ii) {
                        ...
                        if (argExpected !== null && argExpected.isByReference) {
                            ...
                            if (varRef === null) {
                                this.Error(`Expected variable name to pass by reference to 'ref ${argExpected.name}' but saw ${argToPass.ToString()}`);
                                break;
                            }
        ...
        this.PathAsVariableName = () => (this.target.firstComponent);
        this.ResolveTargetContent = () => {
                ...
                if (variableTargetName != null) {
                    ...
                    if (resolveResult.found) {
                        ...
                        if (resolveResult.isArgument) {
                            const argument = resolveResult.ownerFlow.args.find(({ name }) => (name === variableTargetName));
                            if (!argument.isDivertTarget) {
                                this.Error(`Since '${argument.name}' is used as a variable divert target (on ${this.debugMetadata}), it should be marked as: -> ${argument.name}`, resolveResult.ownerFlow);
                            }
                        }
        ...
        this.ResolveReferences = (context) => {
            ...
            var targetFlow = this.targetContent;
            if (targetFlow) {
                if (!targetFlow.isFunction && this.isFunctionCall) {
                    super.Error(`${targetFlow.name} hasn't been marked as a function, but it's being called as one. Do you need to delcare the knot as '== function ${targetFlow.name} =='?`);
                }
            ...
            if (this.target.numberOfComponents == 1) {
                ...
                if (isBuiltIn || isExternal) {
                    ...
                    if (isExternal) {
                        ...
                        this.runtimeDivert.targetPath = new RuntimePath({ componentsString: this.target.firstComponent });
        ...
        this.CheckArgumentValidity = () => {
            ...
            for (let ii = 0; ii < paramCount; ++ii) {
                ...
                if (flowArg.isDivertTarget) {
                    ...
                    var varRef = divArgExpr;
                    if (!(divArgExpr instanceof DivertTarget) && varRef === null) {
                        this.Error(`Target '${targetFlow.name}' expects a divert target for the parameter named -> ${flowArg.name} but saw ${divArgExpr}`, divArgExpr);
                    }
                    else if (varRef !== null) {

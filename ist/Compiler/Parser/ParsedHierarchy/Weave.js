...
export class Weave extends Object {
    constructor(cont, indentIndex = -1) {
        ...
        this.ResolveWeavePointNaming = () => {
            ...
            for (const weavePoint of namedWeavePoints) {
                ...
                if (existingWeavePoint = this._namedWeavePoints[weavePoint.name]) {
                    ...
                    this.Error(`A ${typeName} with the same label name '${weavePoint.name}' already exists in this context on line ${existingObj.debugMetadata.startLineNumber}`, weavePoint);
                }
        ...
        this.ValidateTermination = (badTerminationHandler) => {
            ...
            const hasLooseEnds = this.looseEnds !== null &&
        ...
        this.BadNestedTerminationHandler = (terminatingObj) => {
            ...
            // }
            if (conditional !== null) {
                let numChoices = conditional.FindAll().length;
                if (numChoices === 1) {
        ...
        this.ValidateFlowOfObjectsTerminates = (objFlow, defaultObj, badTerminationHandler) => {
            ...
            for (const flowObj of objFlow) {
                const divert = flowObj.Find((d) => (!d.isThread &&
                    !d.isTunnel &&
                    !d.isFunctionCall &&
        ...
        this.WeavePointHasLooseEnd = (weavePoint) => {
            ...
            for (let ii = weavePoint.content.length - 1; ii >= 0; --ii) {
                var innerDivert = weavePoint.content[ii];
                if (innerDivert) {

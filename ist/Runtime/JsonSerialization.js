...
export class JsonSerialization {
    ...
    static get controlCommandNames() {
        return this._controlCommandNames;
    }
}
JsonSerialization._controlCommandNames = [];
JsonSerialization.JArrayToRuntimeObjList = (jArray, skipLast = false) => {
    let count = jArray.length;
    if (skipLast) {
        count -= 1;
    }
    const list = new Array(jArray.length);
...
JsonSerialization.WriteDictionaryRuntimeObjs = (dictionary) => ({ ...dictionary });
JsonSerialization.WriteListRuntimeObjs = (list) => (list.map((val) => JsonSerialization.WriteRuntimeObject(val)));
JsonSerialization.WriteIntDictionary = (dict) => (Array.from(dict.entries()).reduce((dict, val) => {
    dict[val[0]] = dict[1];
    return dict;
}, {}));
JsonSerialization.WriteRuntimeObject = (obj) => {
    const writer = {};
    const container = obj;
    if (container) {
    ...
    if (divert) {
        ...
        let targetStr = divert.targetPathString;
...
JsonSerialization.JObjectToDictionaryRuntimeObjs = (jObject) => {
    const dict = {};
...
JsonSerialization.JObjectToIntDictionary = (jObject) => {
    const dict = {};
    for (const key in jObject) {
...
// 
// Tag:            {"#": "the tag text"}
JsonSerialization.JTokenToRuntimeObject = (token) => {
    ...
    if (typeof token === 'string') {
        ...
        for (let ii = 0; ii < JsonSerialization.controlCommandNames.length; ++ii) {
            let cmdName = JsonSerialization.controlCommandNames[ii];
            if (str === cmdName) {
                return new RuntimeControlCommand(ii);
            }
        }
        // Native functions
        ...
        if (propValue = obj['^->']) {
            return new DivertTargetValue(new RuntimePath({ componentsString: String(obj['^->']) }));
        }
...
JsonSerialization.WriteRuntimeContainer = (container, withoutName = false) => {
    const writer = [];
    ...
    if (namedOnlyContent) {
        for (const key in namedOnlyContent) {
            const namedContainer = namedOnlyContent[key];
...
JsonSerialization.JArrayToContainer = (jArray) => {
    ...
    const terminatingObj = jArray[jArray.length - 1];
    if (terminatingObj) {
        const namedOnlyContent = new Map();
        for (const key in terminatingObj) {
            ...
            if (key === '#f') {
                ...
                const namedSubContainer = namedContentItem;
...
JsonSerialization.JObjectToChoice = (jObj) => {
...
JsonSerialization.WriteChoice = (choice) => {
    const writer = {};
...
JsonSerialization.WriteInkList = (listVal) => {
    const writer = {};
...
JsonSerialization.JTokenToListDefinitions = (obj) => {
    const defsObj = obj;
    const allDefs = [];
    for (const key in defsObj) {
        ...
        const def = new RuntimeListDefinition(name, new Map(Object.keys(items).map((key) => [key, items[key]])));
        allDefs.push(def);
    }

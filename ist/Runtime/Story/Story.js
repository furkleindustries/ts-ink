...
import { RuntimeDivert, } from '../Divert/Divert';
import { DivertTargetValue, } from '../Value/DivertTargetValue';
...
import { RuntimeVariableAssignment, } from '../Variable/VariableAssignment';
import { VariablePointerValue, } from '../Value/VariablePointerValue';
import { RuntimeVariableReference, } from '../Variable/VariableReference';
...
export class RuntimeStory extends RuntimeObject {
    constructor({ contentContainer, lists = null, jsonString, } = {}) {
        super();
        ...
        this.GetSerializedRepresentation = (story) => ({
            ...
            listDefs: story.listDefinitions ?
                story.listDefinitions.lists.reduce((obj, { name, items, }) => Object.assign(obj, { [name]: items })) :
                null,
        });
        /// <summary>
        ...
        this.ResetGlobals = () => {
            if ('global decl' in this.mainContentContainer.namedContent) {
                ...
                this.ChoosePath(new RuntimePath({ componentsString: 'global decl' }), false);
        ...
        /// </summary>
        this.ContinueAsync = (millisecsLimitAsync) => {
            if (!this.hasValidatedExternals) {
                this.ValidateExternalBindings();
            }
            this.ContinueInternal(millisecsLimitAsync);
        };
        this.ContinueInternal = (millisecsLimitAsync = 0) => {
            if (this.profiler) {
        ...
        this.ContinueSingleStep = () => {
            ...
            if (!this.state.inStringEvaluation) {
                ...
                if (this.stateSnapshotAtLastNewline) {
                    ...
                    const change = this.CalculateNewlineOutputStateChange(this.stateSnapshotAtLastNewline.currentText, this.state.currentText, this.stateSnapshotAtLastNewline.currentTags.length, this.currentTags.length);
        ...
        this.CalculateNewlineOutputStateChange = (prevText, currText, prevTagCount, currTagCount) => {
        ...
        this.ContentAtPath = (path) => (this.mainContentContainer.ContentAtPath(path));
        this.KnotContainerWithName = (name) => {
            const namedContent = this.mainContentContainer.namedContent;
            const namedContainer = namedContent[name];
            if (namedContainer) {
                return namedContainer;
            }
            return null;
        };
        this.PointerAtPath = (path) => {
            if (path.length === 0) {
                return Pointer.Null;
            }
        ...
        this.CopyStateForBackgroundThreadSave = () => {
        ...
        this.Step = () => {
            ...
            let containerToEnter = pointer.Resolve();
            ...
            const choicePoint = currentContentObj;
            if (choicePoint) {
            ...
            if (shouldAddToStream) {
                ...
                const varPointer = currentContentObj;
        ...
        this.VisitChangedContainersDueToDivert = () => {
            ...
            if (!previousPointer.isNull) {
                let prevAncestor = previousPointer.Resolve() ||
            ...
            let currentContainerAncestor = currentChildOfContainer.parent;
        ...
        this.ProcessChoice = (choicePoint) => {
            3;
            let showChoice = true;
            ...
            if (choicePoint.hasChoiceOnlyContent) {
                const choiceOnlyStrVal = this.state.PopEvaluationStack();
                choiceOnlyText = choiceOnlyStrVal.value;
            }
            if (choicePoint.hasStartContent) {
                const startStrVal = this.state.PopEvaluationStack();
        ...
        this.PerformLogicAndFlowControl = (contentObj) => {
            if (contentObj === null) {
                return false;
            }
            // Divert
            if (contentObj instanceof RuntimeDivert) {
                ...
                if (currentDivert.pushesToStack) {
                    this.state.callStack.Push(currentDivert.stackPushType, this.state.outputStream.length);
                }
                ...
                switch (evalCommand.commandType) {
                    ...
                    case CommandType.EvalOutput:
                        ...
                        if (this.state.evaluationStack.length > 0) {
                            ...
                            if (!(output instanceof Void)) {
                                ...
                                const val = 'ToString' in output ? output.ToString() : String(output);
                    ...
                    case CommandType.PopTunnel:
                        ...
                        if (popType === PushPopType.Tunnel) {
                            ...
                            const overrideTunnelReturnTarget = popped;
                            if (overrideTunnelReturnTarget === null) {
                                if (!(popped instanceof Void)) {
                                    throw new Error('Expected void if ->-> doesn\'t override target.');
                                }
                            }
                        }
                    ...
                    case CommandType.BeginString:
                        ...
                        if (!this.state.inExpressionEvaluation) {
                            throw new Error('Expected to be in an expression when evaluating a string');
                        }
                        this.state.inExpressionEvaluation = false;
                        break;
                    case CommandType.EndString:
                        ...
                        for (const c of contentStackForString) {
                            sb += ('ToString' in c) ? c.ToString() : String(c);
                        }
                    ...
                    case CommandType.ReadCount:
                        ...
                        const divertTarget = target;
                        const container = this.ContentAtPath(divertTarget.targetPath).correctObj;
                        let eitherCount;
                        if (container) {
                    ...
                    case CommandType.Random: {
                        const maxInt = this.state.PopEvaluationStack();
                        const minInt = this.state.PopEvaluationStack();
                        if (!minInt) {
                    ...
                    case CommandType.SeedRandom:
                        const seed = this.state.PopEvaluationStack();
                        if (!seed) {
                    ...
                    case CommandType.VisitIndex:
                        const count = this.state.VisitCountForContainer(this.state.currentPointer.container) - 1; // index not count
                    ...
                    case CommandType.ListFromInt:
                        const intVal = this.state.PopEvaluationStack();
                        const listNameVal = this.state.PopEvaluationStack();
                        if (intVal === null) {
                            throw new StoryError('Passed non-integer when creating a list element from a numerical value.');
                        }
                        ...
                        if (foundListDef) {
                            ...
                            throw new StoryError(`Failed to find LIST called ${listNameVal.value}`);
                        }
                    ...
                    case CommandType.ListRange: {
                        const max = this.state.PopEvaluationStack();
                        const min = this.state.PopEvaluationStack();
                        const targetList = this.state.PopEvaluationStack();
                        if (!targetList || !min || !max) {
                            throw new StoryError('Expected list, minimum and maximum for LIST_RANGE');
                        }
                    ...
                    case CommandType.ListRandom: {
                        const listVal = this.state.PopEvaluationStack();
                        if (listVal === null) {
                ...
                const funcParams = this.state.PopEvaluationStack(func.numberOfParameters);
        ...
        this.ChoosePathString = (path, resetCallstack = true, ...args) => {
        ...
        this.IfAsyncWeCant = (activityStr) => {
            if (this.asyncContinueActive) {
        ...
        this.ChoosePath = (p, incrementingTurnIndex = true) => {
        ...
        this.EvaluateFunction = (functionName, ...args) => {
            ...
            const outputStreamBefore = [...this.state.outputStream];
            this.state.ResetOutput();
        ...
        // CommandLinePlayer.
        this.EvaluateExpression = (exprContainer) => {
        ...
        this.CallExternalFunction = (funcName, numberOfArguments) => {
            const func = this.externals[funcName];
            ...
            if (!func) {
                if (this.allowExternalFunctionFallbacks) {
                    ...
                    if (!fallbackFunctionContainer) {
                        throw new Error(`Trying to call EXTERNAL function '${funcName}' which has not been bound, and fallback ink function could not be found.`);
                    }
                    // Divert direct into fallback function and we're done
                    this.state.callStack.Push(PushPopType.Function, this.state.outputStream.length);
                    this.state.divertedPointer = Pointer.StartOf(fallbackFunctionContainer);
                    return;
                }
                throw new Error(`Trying to call EXTERNAL function '${funcName}' which has not been bound (and ink fallbacks disabled).`);
            }
            // Pop arguments
            const args = [];
            for (let ii = 0; ii < numberOfArguments; ++ii) {
                const poppedObj = this.state.PopEvaluationStack();
            ...
            if (funcResult) {
                ...
                if (!returnObj !== null) {
                    throw new Error(`Could not create ink value from returned object of type ${funcResult.GetType()}`);
                }
            }
            else {
                returnObj = new Void();
        ...
        this.BindExternalFunctionGeneral = (funcName, func) => {
        ...
        this.TryCoerce = (value, T) => {
            if (value === null || value === undefined) {
                return null;
            }
            const sameType = ((typeof value === 'number' && T === 'float') ||
            ...
            throw new Error(`Failed to cast ${value.GetType()} to ${typeof T}`);
        };
        ...
        /// <param name="funcName">EXTERNAL ink function name to bind to.</param>
        /// <param name="func">The C# function to bind.</param>
        this.BindExternalFunction = (funcName, func, ...args) => {
        ...
        /// </summary>
        this.UnbindExternalFunction = (funcName) => {
        ...
        this._validateWithObj = (obj, missingExternals) => {
        ...
        /// </summary>
        this.ValidateExternalBindings = (c, missingExternals = new Set()) => {
            if (c && missingExternals) {
                for (const innerContent of c.content) {
                    const container = innerContent;
                    if (!container || !container.hasValidName) {
                        this._validateWithObj(innerContent, missingExternals);
                    }
                }
                for (const value of Object.values(c.namedContent)) {
                    this._validateWithObj(value, missingExternals);
                }
            }
            else {
        ...
        this.ObserveVariable = (variableName, observer) => {
            this.IfAsyncWeCant('observe a new variable');
            if (!this.variableObservers) {
                this._variableObservers = new Map();
            }
            if (!this.state.variablesState.GlobalVariableExistsWithName(variableName)) {
                throw new StoryError(`Cannot observe variable '${variableName}' because it wasn't declared in the ink story.`);
            }
        ...
        this.ObserveVariables = (variableNames, observer) => {
        ...
        this.RemoveVariableObserver = (observer = null, specificVariableName = null) => {
            ...
            if (specificVariableName) {
                if (this.variableObservers.has(specificVariableName)) {
                    if (observer) {
                        this.variableObservers.get(specificVariableName).splice(this.variableObservers.get(specificVariableName).indexOf(observer), 1);
                ...
                for (const varName of keys) {
                    this.variableObservers.get(varName).splice(this.variableObservers.get(varName).indexOf(observer), 1);
        ...
        this.VariableStateDidChangeEvent = (variableName, newValueObj) => {
            if (!this.variableObservers) {
                return;
            }
        ...
        this.TagsForContentAtPath = (path) => (this.TagsAtStartOfFlowContainerWithPathString(path));
        this.TagsAtStartOfFlowContainerWithPathString = (pathString) => {
        ...
        /// </summary>
        this.BuildStringOfHierarchy = () => (this.mainContentContainer.BuildStringOfHierarchy('', 0, this.state.currentPointer.Resolve()));
        this.BuildStringOfContainer = (container) => (container.BuildStringOfHierarchy('', 0, this.state.currentPointer.Resolve()));
        this.NextContent = () => {
        ...
        this.IncrementContentPointer = () => {
            ...
            while (pointer.index >= pointer.container.content.length) {
                ...
                const nextAncestor = pointer.container.parent;
                if (!nextAncestor) {
                    break;
                }
        ...
        // then exits the flow.
        this.Error = (message, useEndLineNumber = false) => {
        ...
        this.AddError = (msg, isWarning = false, useEndLineNumber = false) => {
        ...
        this.Assert = (condition, message = 'Story assert', ...formatParams) => {
            if (!condition) {
                if (formatParams.length) {
                    message += formatParams.map((param) => ('ToString' in param ? param.ToString() : String(param))).join('');
                }
        ...
        if (jsonString) {
            ...
            let rootObject = JSON.parse(jsonString);
            let versionNumber = rootObject.inkVersion;
            if (!versionNumber) {
            ...
            const listDefsObj = rootObject.listDefs;
            if (listDefsObj) {
                this._listDefinitions = JsonSerialization.JTokenToListDefinitions(listDefsObj);
            }
            this._mainContentContainer = JsonSerialization.JTokenToRuntimeObject(rootToken);
            this.ResetState();
        }
    ...
    /// </summary>
    get inkVersionMinimumCompatible() {
        return 18;
    }
    /// <summary>
    ...
    get nextSequenceShuffleIndex() {
        ...
        const seqCountVal = this.state.PopEvaluationStack();
